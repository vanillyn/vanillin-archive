...
module.exports = {
  ...
  async execute(interaction) {
    const result = await request(
      `https://gelbooru.com/index.php?page=dapi&s=post&q=index&limit=1&tags=sort:random+1girl+-rating:explicit+-rating:questionable+-rating:sensitive+-1boy+-2boys+-3boys+-4boys+-5boys&json=1&api_key=${auth.gelbooru.key}&user_id=${auth.gelbooru.id}`
    );
    const file = await result.body.json();
    const gpost = file.post[0];
    console.log(file);
