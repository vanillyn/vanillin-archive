...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('game')
		.setDescription('search for a user on steam using their vanity url or their steamid!')
		.addStringOption(option =>
			option.setName('vanity')
				  .setDescription('their vanity url. make sure it is just the name, remove the "https://steamcommunnity.com/id/"')
				  )
		.addNumberOption( option => 
			option.setName('steamid')
				  .setDescription('their steam id, you can find it by going to their profile and copying the number after "/id/')),
	async execute(interaction) {
		const sent = await interaction.reply({ content: 'Searching Steam... <a:AquaJamming:1041197752157229147>', fetchReply: true });
		const name = interaction.options.getString('vanity') ?? interaction.options.getNumber('steamid');
		if (name == interaction.options.getString('vanity')) {
			const games = await request(`https://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${auth.steamkey}`);
