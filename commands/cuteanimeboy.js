...
module.exports = {
  ...
  async execute(interaction) {
    const result = await request(
      `https://gelbooru.com/index.php?page=dapi&s=post&q=index&limit=1&tags=sort:random+1boy+trap+-rating:explicit+-rating:questionable+-rating:sensitive+-1girl+-2girls+-3girls+-4girls+-5girls+-child&json=1&api_key=${auth.gelbooru.key}&user_id=${auth.gelbooru.id}`
    );
    const file = await result.body.json();
    const gpost = file.post[0];
    console.log(file);
